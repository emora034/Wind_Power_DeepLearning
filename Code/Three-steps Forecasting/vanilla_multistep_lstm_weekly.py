...
def run_lstm(train_X, train_Y, test_X, test_Y, epochs_no, batch_no, neurons):
    ...
    model0.add(LSTM(neurons, activation='relu',return_sequences=True, input_shape=(train_X.shape[1],1)))
    model0.add(Flatten())
...
rmse0.to_csv(r'Weekly_3stepLSTM_Results.csv',index=False, header=True)