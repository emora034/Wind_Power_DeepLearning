...
#load datasets
datafile = "/Users/emora034/OneDrive/Desktop/TFM/Wind Power - Modeling & Forecasting - Resources/Code/data.npz"
...
def run_lstm(train_X, train_Y, test_X, test_Y, epochs_no, batch_no, neurons):
    ...
    model0.add(LSTM(neurons, activation='relu', input_shape=(train_X.shape[1],1)))
    #repeat vector takes the number of output steps defined through the data_split function
    model0.add(RepeatVector(steps_out))
    model0.add(LSTM(neurons, activation='relu', return_sequences=True))
    #model0.add(TimeDistributed(Flatten()))
    #dense layer defined as 1 since the 3-step output is contained in 1 vector
    model0.add(TimeDistributed(Dense(1)))
    model0.compile(loss='mean_squared_error', optimizer= Adam(epsilon=1e-03), metrics=['mean_absolute_percentage_error'])
    #fit model
    ...
    #reshape and scale back to original scale
    ...
    train_YS=scaler.inverse_transform(np.reshape(train_Y, (train_Y.shape[0], train_Y.shape[1])))
    test_YS = scaler.inverse_transform(np.reshape(test_Y, (test_Y.shape[0], test_Y.shape[1])))
...
#rmse0.to_csv(r'Weekly_AutoLSTM_Results.csv',index=False, header=True)

######## j Epochs, k Neurons, i batches ###########
# epsilon at -3 same as above
        #Weekly split 24x7 - 720
#define batches, epochs, and neurons to be implemented
K=[24] #batches
L=[100] #epochs
M=[32] #neurons 
...
#model to be ran given the different time "slicings"
def run_lstm_op(train_X, train_Y, test_X, test_Y, epochs_no, batch_no, neurons):
    #prepare model
    model0 = Sequential()
    #input shape refers to the time steps to learn from and the columns of the data
    #therefore, for univariate data (steps_in, 1)
    ...
    #dense layer defined as 1 since the 3-step output is contained in 1 vector
    model0.add(TimeDistributed(Dense(1)))
    model0.compile(loss='mean_squared_error', optimizer= Adam(epsilon=1e-02), metrics=['mean_absolute_percentage_error'])
    #fit model
    model0.fit(train_X, train_Y, epochs=epochs_no, batch_size=batch_no, verbose=2) 
...
j_epochs,i_batch, k_neurons= list(),list(),list()
for i,j,k in itertools.product(K, L, M):
    run_lstm_op(train_X, train_Y, test_X, test_Y,
             epochs_no=j, batch_no=i, neurons=k)
    rmse0=rmse0.append([rmse_report])
    i_batch.append(i)
    j_epochs.append(j)
