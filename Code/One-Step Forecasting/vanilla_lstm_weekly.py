...
def run_lstm(train_X, train_Y, test_X, test_Y, epochs_no, batch_no, neurons):
    #prepare model
    model0 = Sequential()
    model0.add(LSTM(neurons, activation='relu',return_sequences=True, input_shape=(1,train_X.shape[2])))
    model0.add(Flatten())
    model0.add(Dense(1))
...
rmse0.to_csv(r'Weekly_LSTM_Results.csv',
            index=False, header=True)

#record df index where the lowest MAPE is recorded
#rmse0=pd.read_excel("/content/sample_data/Weekly_VanSLTM.xlsx")
...
plt.legend(["True Values","Training Prediction","Test Prediction"])
plt.ylabel('MWh')
plt.show()
...
# plot baseline and predictions
import seaborn as sns
sns.set(rc={'figure.figsize':(15,5)}, font_scale = 1.8)
fig, ax=plt.subplots()
