...
test_train_reshape(train,test,720,24)
...
rmse0.to_csv(r'Monthly_CNNLSTM_Results.csv',
...
#read excel table with results
#rmse0=pd.read_excel("/content/sample_data/Monthly_CNNLSTM_Results.xlsx")
...
run_lstm(train_X, train_Y, test_X, test_Y,
             ...
             neurons=min_df['Neurons'].values[0])
...
testPredictPlot[len(trainPredict)+(720*2)+1:len(ts_scaled)-1, :] = testPredict
trainPredictPlot = np.empty_like(ts_scaled)
trainPredictPlot[:, :] = np.nan
trainPredictPlot[720:len(trainPredict)+720, :] = trainPredict
...
sns.set(rc={'figure.figsize':(15,5)}, font_size=1.8)
fig, ax=plt.subplots()

ax.plot(scaler.inverse_transform(ts_scaled))
ax.plot(trainPredictPlot)
ax.legend(["True Values","Training Prediction"],loc='upper left', frameon=False)
ax.set_ylim([0, 17500])
ax.set_ylabel('MWh')

plt.plot(testPredictPlot)
plt.set_ylim([0, 17500])
plt.set_ylabel('MWh')